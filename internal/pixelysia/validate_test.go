...
func TestValidateRealRepository(t *testing.T) {
	...
	if !strings.Contains(out.String(), "0 failed") || !strings.Contains(out.String(), "0 informational") {
		t.Fatalf("expected a fully clean validation of every repository theme, got:\n%s", out.String())
	}
}
